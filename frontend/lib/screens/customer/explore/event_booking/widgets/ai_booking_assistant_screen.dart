...
class AIBookingAssistantScreen extends StatefulWidget {
  ...
  @override
  State<AIBookingAssistantScreen> createState() =>
      _AIBookingAssistantScreenState();
}
...
class _AIBookingAssistantScreenState extends State<AIBookingAssistantScreen> {
  ...
  List<Map<String, dynamic>> _relevantMenusWithTruckInfo =
      []; // To store menus of fetched trucks
  ...
  String _normalizeCity(String? cityNameFromAI) {
    ...
    for (String supportedCity in supportedBookingCities) {
      if (normalized.contains(supportedCity)) {
        return supportedCity.substring(0, 1).toUpperCase() +
            supportedCity.substring(1);
      }
    }
    print(
        "[AI_BOOKING_DEBUG] City '$cityNameFromAI' not in supported list, using as is.");
    return cityNameFromAI.isNotEmpty
        ? cityNameFromAI.substring(0, 1).toUpperCase() +
            cityNameFromAI.substring(1)
        : widget.initialCity ?? "Qalqilya";
  }
  ...
  Future<void> _getAIAssistance() async {
    ...
    try {
      _extractedBookingParams =
          await _extractBookingParametersFromAI(userQuery);
      print("[AI_ASSIST_PARAMS] Extracted by AI: $_extractedBookingParams");

      if (!mounted) return;
      if (_extractedBookingParams.isEmpty ||
          _extractedBookingParams['error'] != null) {
        setState(() {
          _aiResponseText = _extractedBookingParams['error'] ??
              "I couldn't understand. Try details like date, guests, city.";
          _isLoading = false;
        });
        return;
      }

      String cityExtractedByAI =
          _extractedBookingParams['city'] as String? ?? '';
      final String cityToSearch = _normalizeCity(cityExtractedByAI.isNotEmpty
          ? cityExtractedByAI
          : widget.initialCity);
      _extractedBookingParams['city'] = cityToSearch;

      print(
          "[AI_ASSIST_CITY_SEARCH] Normalized City for Truck Search: $cityToSearch");

      final List<dynamic> rawTrucks =
          await TruckOwnerService.getPublicTrucks(city: cityToSearch);
      _fetchedTrucks =
          rawTrucks.map((truck) => truck as Map<String, dynamic>).toList();
      print(
          "[AI_ASSIST_TRUCKS_FETCHED] Count of Trucks Fetched in '$cityToSearch': ${_fetchedTrucks.length}");

      if (!mounted) return;
      if (_fetchedTrucks.isEmpty) {
        setState(() {
          _aiResponseText =
              "Sorry, no trucks found in $cityToSearch for your request. Try another city or adjust criteria.";
          _isLoading = false;
        });
        return;
      }
      ...
      final http.Response allMenusResponse =
          await MenuService.getAllMenusWithTrucks();
      if (allMenusResponse.statusCode == 200) {
        final List<dynamic> allMenusData =
            jsonDecode(allMenusResponse.body) as List<dynamic>? ?? [];
        final Set<String> fetchedTruckIds =
            _fetchedTrucks.map((t) => t['_id'] as String).toSet();

        _relevantMenusWithTruckInfo = allMenusData
            .where((menuGroup) =>
                fetchedTruckIds.contains(menuGroup['truckId'] as String?))
            .map((menuGroup) {
              ...
              if (menuGroup['menu'] is List) {
                for (var item in (menuGroup['menu'] as List)) {
                  if (item is Map<String, dynamic>) {
                    ...
                    newItem['truck_id_for_ai'] = menuGroup[
                        'truckId']; // Use original truckId from menuGroup
            ...
            .expand((list) =>
                list) // Flatten the list of lists into a single list of menu items
            .toList();
        print(
            "[AI_ASSIST_MENUS] Relevant Menu Items Count: ${_relevantMenusWithTruckInfo.length}");
      } else {
        print(
            "[AI_ASSIST_MENUS] Failed to fetch all menus: ${allMenusResponse.statusCode}");
      ...
      final List<Map<String, dynamic>> suggestedOptionsFromAI =
          await _generateBookingOptionsFromAI(_extractedBookingParams,
              _fetchedTrucks, _relevantMenusWithTruckInfo // Pass relevant menus
              );
      print(
          "[AI_ASSIST_SUGGESTIONS] AI Suggested Options (Raw from AI): $suggestedOptionsFromAI");
      ...
      if (suggestedOptionsFromAI.isNotEmpty) {
        if (suggestedOptionsFromAI.first['truck_id'] == null &&
            suggestedOptionsFromAI.first['reason'] != null) {
          responseMessageToShow = suggestedOptionsFromAI.first['reason'] ??
              "The AI indicated no specific options could be found.";
        } else {
          optionsToDisplay = suggestedOptionsFromAI;
          responseMessageToShow = "Great! I found these options for you:";
        }
      } else {
        responseMessageToShow =
            "I looked, but couldn't find suitable booking options. You could try being more flexible.";
      }
      ...
      setState(() {
        _aiResponseText = "An error occurred. Please try again.";
        _isLoading = false;
      });
    }
  }

  Future<Map<String, dynamic>> _extractBookingParametersFromAI(
      ...
      if (response.statusCode == 200) {
        ...
        if (content != null) {
          ...
          try {
            ...
            print("[AI_BOOKING_DEBUG] Error decoding params JSON: $e");
            return {'error': "AI response format error."};
          }
        }
      }
      print(
          "[AI_BOOKING_DEBUG] Params extraction failed: ${response.statusCode} - ${response.body}");
      return {'error': "AI parameter extraction failed."};
    } catch (e) {
      print("[AI_BOOKING_DEBUG] Params extraction communication error: $e");
      return {'error': "AI communication error."};
    }
  }
  ...
  Future<List<Map<String, dynamic>>> _generateBookingOptionsFromAI(
    ...
    final summarizedTrucks = allFetchedTrucks
        .map((truck) => {
              'truck_id': truck['_id'], 'truck_name': truck['truck_name'],
              'cuisine_type': truck['cuisine_type'],
              'description': (truck['description'] as String?)?.substring(
                      0,
                      (truck['description'] as String?)!.length > 60
                          ? 60
                          : (truck['description'] as String?)!.length) ??
                  "",
              'operating_hours': truck['operating_hours'],
              'unavailable_dates':
                  (truck['unavailable_dates'] as List<dynamic>? ?? [])
    ...
    final summarizedRelevantMenus = relevantMenus
        .map((item) => {
              'menu_item_id': item['_id'],
              'menu_item_name': item['name'],
              'truck_id': item[
                  'truck_id_for_ai'], // ID of the truck this item belongs to
    ...
    try {
      final response = await http
          .post(
            _geminiUrl, headers: {'Content-Type': 'application/json'},
            body: jsonEncode({
              'contents': [
                {
                  'parts': [
          ...
          .timeout(const Duration(
              seconds: 45)); // Increased timeout for more complex prompt
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        final content = data['candidates']?[0]?['content']?['parts']?[0]
            ?['text'] as String?;
        if (content != null) {
          ...
          try {
            ...
            print(
                "[AI_BOOKING_DEBUG] Error decoding options JSON: $e \nContent: $content");
            return [];
          }
        }
      }
      print(
          "[AI_BOOKING_DEBUG] Options generation failed: ${response.statusCode} - ${response.body}");
      return [
        {
          "reason":
              "AI service failed to generate options (Status: ${response.statusCode})."
        }
      ];
    } catch (e) {
      print("[AI_BOOKING_DEBUG] Options generation communication error: $e");
      return [
        {"reason": "Could not reach AI service to generate options."}
      ];
    }
  }

  void _navigateToFinalBookingForm(
      Map<String, dynamic> truckData, Map<String, dynamic> selectedOption) {
    ...
    try {
      if (selectedOption['suggested_date'] != null) {
        parsedStartDate = DateFormat('yyyy-MM-dd')
            .parseStrict(selectedOption['suggested_date']);
        parsedEndDate = parsedStartDate;
      }
      if (selectedOption['suggested_start_time'] != null) {
        initialStartTime = TimeOfDay.fromDateTime(DateFormat('hh:mm a')
            .parse(selectedOption['suggested_start_time']));
      }
      if (selectedOption['suggested_end_time'] != null) {
        initialEndTime = TimeOfDay.fromDateTime(
            DateFormat('hh:mm a').parse(selectedOption['suggested_end_time']));
      }
    } catch (e) {
      print("Error parsing AI date/time: $e");
    }
    int? initialGuestCountValue;
    final dynamic rawGuestCount = _extractedBookingParams['guest_count'];
    if (rawGuestCount != null) {
      if (rawGuestCount is String) {
        initialGuestCountValue = int.tryParse(rawGuestCount);
      } else if (rawGuestCount is int)
        initialGuestCountValue = rawGuestCount;
      else if (rawGuestCount is double)
        initialGuestCountValue = rawGuestCount.toInt();
    }
    String? specialRequestsFromAI =
        _extractedBookingParams['special_requests'] as String?;
    if (specialRequestsFromAI != null &&
        specialRequestsFromAI.trim().isNotEmpty) {
      final List<String> commonPhrasesToIgnore = [
        "food truck",
        "book a truck",
        "truck for",
        "event",
      ...
      bool containsCommonPhrase = commonPhrasesToIgnore.any(
          (phrase) => specialRequestsFromAI!.toLowerCase().contains(phrase));
      List<String> cuisinePrefsLower =
          ((_extractedBookingParams['cuisine_preferences'] as List<dynamic>?)
                  ?.map((e) => e.toString().toLowerCase())
                  .toList() ??
              []);
      bool containsCuisine = cuisinePrefsLower
          .any((c) => specialRequestsFromAI!.toLowerCase().contains(c));
      if (containsCommonPhrase || containsCuisine) {
        final List<String> dietaryKeywords = [
          "vegan",
          "nut-free",
          "gluten-free",
          "vegetarian",
        ...
        bool containsDietaryKeyword = dietaryKeywords.any((keyword) =>
            specialRequestsFromAI!.toLowerCase().contains(keyword));
        if (!containsDietaryKeyword) specialRequestsFromAI = null;
      }
    } else {
      specialRequestsFromAI = null;
    }
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => FinalBookingForm(
          ...
          initialGuestCount: initialGuestCountValue,
          initialLocation: _extractedBookingParams['city'] as String?,
          initialSpecialRequests: specialRequestsFromAI,
        ),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
          title: const Text("AI Event Booking Assistant"),
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            Text("Tell me about your event!",
                style: Theme.of(context).textTheme.titleLarge?.copyWith(
                    color: ffOnSurfaceColor, fontWeight: FontWeight.w600),
                textAlign: TextAlign.center),
            const SizedBox(height: ffPaddingSm),
            Text(
                "e.g., 'Book a pizza truck for 30 people in Jenin next Friday.'",
                style: Theme.of(context)
                    .textTheme
                    .bodySmall
                    ?.copyWith(color: ffSecondaryTextColor),
                textAlign: TextAlign.center),
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                hintStyle:
                    TextStyle(color: ffSecondaryTextColor.withOpacity(0.7)),
                ...
                focusedBorder: OutlineInputBorder(
                    borderRadius: BorderRadius.circular(ffBorderRadius),
                    borderSide:
                        const BorderSide(color: ffPrimaryColor, width: 1.5)),
                filled: true,
                fillColor: ffSurfaceColor,
                suffixIcon: IconButton(
                  icon: Icon(Icons.send_rounded,
                      color: _isLoading ? Colors.grey : ffPrimaryColor),
                  onPressed: _isLoading ? null : _getAIAssistance,
                ),
              ),
              textInputAction: TextInputAction.send,
              onSubmitted: _isLoading ? null : (_) => _getAIAssistance(),
              maxLines: 3,
              minLines: 1,
            ),
            ...
            if (_isLoading)
                    ...
                    Expanded(
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          if (option['truck_id'] == null &&
                              option['reason'] != null) {
                            return Card(
                                elevation: 2,
                                margin: const EdgeInsets.only(
                                    bottom: ffPaddingMd - 4),
                                shape: RoundedRectangleBorder(
                                    borderRadius: BorderRadius.circular(
                                        ffBorderRadius - 2)),
                                child: ListTile(
                                  contentPadding: const EdgeInsets.symmetric(
                                      horizontal: ffPaddingMd - 4,
                                      vertical: ffPaddingSm - 2),
                                  ...
                                  title: Text("Information",
                                      style: TextStyle(
                                          ...
                                          color:
                                              ffOnSurfaceColor.withOpacity(0.9),
                                          fontSize: 16)),
                          ...
                          final truckData = _fetchedTrucks.firstWhere(
                              (t) => t['_id'] == option['truck_id'],
                              orElse: () => {
                                    'truck_name':
                                        option['truck_name'] ?? "Unknown Truck",
                                    '_id': option['truck_id']
                                  });
                          return Card(
                            elevation: 2,
                            margin:
                                const EdgeInsets.only(bottom: ffPaddingMd - 4),
                            shape: RoundedRectangleBorder(
                                borderRadius:
                                    BorderRadius.circular(ffBorderRadius - 2)),
                            child: ListTile(
                              contentPadding: const EdgeInsets.symmetric(
                                  horizontal: ffPaddingMd - 4,
                                  vertical: ffPaddingSm - 2),
                              leading: CircleAvatar(
                                  backgroundColor:
                                      ffPrimaryColor.withOpacity(0.15),
                                  child: const Icon(
                              ...
                              title: Text(
                                  option['truck_name'] ?? 'Truck Option',
                                  style: TextStyle(
                                      fontWeight: FontWeight.w600,
                                      color: ffOnSurfaceColor.withOpacity(0.9),
                                      fontSize: 16)),
                              subtitle: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    ...
                                    Text(
                                        "📅 Date: ${option['suggested_date'] ?? 'N/A'}",
                                        style: const TextStyle(
                                            fontSize: 13,
                                            color: ffSecondaryTextColor)),
                                    Text(
                                        "🕒 Time: ${option['suggested_start_time'] ?? 'N/A'} - ${option['suggested_end_time'] ?? 'N/A'}",
                                        style: const TextStyle(
                                            fontSize: 13,
                                            color: ffSecondaryTextColor)),
                                    if (option['reason'] != null &&
                                        (option['reason'] as String)
                                            .isNotEmpty) ...[
                                      const SizedBox(height: ffPaddingXs),
                                      Text("✨ Why: ${option['reason']}",
                                          style: TextStyle(
                                              fontSize: 12.5,
                              ...
                              trailing: Icon(Icons.arrow_forward_ios_rounded,
                                  size: 18,
                                  color: ffSecondaryTextColor.withOpacity(0.8)),
                              onTap: () {
                                if (truckData.isNotEmpty &&
                                    truckData['_id'] != null) {
                                  _navigateToFinalBookingForm(
                                      truckData, option);
                                } else {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(
                                          content: Text(
                                              "Details for truck option incomplete.")));
                                }
                              },
                            ),
                          );
              ...
              Expanded(
                ...
                child: Text(_aiResponseText,
                    textAlign: TextAlign.center,
                    style: const TextStyle(
                        fontSize: 16, color: ffSecondaryTextColor)),
              )))
          ],
        ),
      ),
